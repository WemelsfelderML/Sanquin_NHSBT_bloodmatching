...
def alloimmunize(SETTINGS, PARAMS, scenario, hospital, day, x):
    ...
    Im = (np.ones(Iv.shape) - Iv)              # I × A matrix with a 1 if the inventory product is negative for some antigen, 0 if positive.
    ...
    for r in r_SCD:                                     # Loop over all requests for SCD patients.
        for i in range(len(I)):                         # Loop over all inventory products.
            for k in A.keys():                          # Loop over all antigens.
                if (k < 3) and (x[i,r] * Im[i,k] * Rv[r,k] > 0):      # If a major antigen is substituted...
                    subst[k] += 1
    ...
    requests_SCD = np.array([[R[r].num_units] + list(R[r].vector) + list(R[r].antibodies) + list(R[r].mism_units) + subst + [R[r].index] for r in r_SCD])
